...
def errors_mc(fitpars,
    ...
    diffarr = np.transpose(
        np.array([sig * fiterrors[1], sig * fiterrors[2], sig * fiterrors[4]]))
    ...
    for i, ff in enumerate(uqcfs):
        ...
        mcpararr[:, theselines,
                 0] = (mcpararr[:, theselines, 0] -
                       diff) + 2. * diff * randarr[:, theselines, 0]
    for i, ff in enumerate(uqbfs):
        ...
        mcpararr[:, theselines,
                 1] = (mcpararr[:, theselines, 1] -
                       diff) + 2. * diff * randarr[:, theselines, 1]
    for i, ff in enumerate(uqvfs):
        ...
        mcpararr[:, theselines,
                 2] = (mcpararr[:, theselines, 2] -
                       diff) + 2. * diff * randarr[:, theselines, 2]

    chisq = np.zeros(numiter)
    for i in range(numiter):
        mcpars = [
            fitpars[0], mcpararr[i, :, 0], mcpararr[i, :, 1], fitpars[3],
            mcpararr[i, :, 2], fitpars[5], fitpars[6]
        ]
